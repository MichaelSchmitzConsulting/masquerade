...
public class OSGiScriptEngineManager extends ScriptEngineManager{
	...
	public OSGiScriptEngineManager(BundleContext context) throws IOException {
	...
	private Map<ScriptEngineManager, ClassLoader> findManagers(BundleContext context) throws IOException {
		...
		for(String factoryName: findFactoryCandidates(context)){
			...
			try {
				...
				System.err.println("Warning: Unable to load script engine factory class " + factoryName + ": " + e.getMessage());
			}
		}
		return managers;
	}
	...
	private List<String> findFactoryCandidates(BundleContext context) throws IOException{
		...
		for (Bundle bundle : bundles) {
			...
			Enumeration<URL> urls = bundle.findEntries("META-INF/services", "javax.script.ScriptEngineFactory", false);
			if (urls == null)
				continue;
